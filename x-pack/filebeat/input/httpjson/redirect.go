...
func convertHttpjsonToCel(cfg *conf.C) (*conf.C, error) {
	...
	// Copy shared fields that map directly or with a rename.
	shared := []struct{ src, dst string }{
		{"interval", "interval"},
	...
	// Extract cel-namespaced fields into their top-level equivalents.
	celFields := []struct{ src, dst string }{
		{"cel.program", "program"},
