...
func TestEDNS0(t *testing.T) {
	...
	var got eventStore
	dns := newDNS(&got, testing.Verbose())
	packet := newPacket(reverse, resp)
	dns.ParseUDP(packet)

	if len(got.events) != 1 {
		t.Fatalf("unexpected number of events, want:1 got:%d", len(got.events))
	}

	// Normalise time in event.end.
	got.events[0].Fields.Put("event.end", time.Time{})

	assert.Equal(t, want, got.events[0].Fields)
}
